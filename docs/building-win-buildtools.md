# Build instructions for Windows 32-bit using Build Tools (without Visual Studio IDE)

This guide shows how to build 64Gram on Windows 32-bit without installing Visual Studio 2022 IDE, using only the free **Build Tools for Visual Studio 2022**. This approach avoids any licensing concerns.
...
All commands (if not stated otherwise) will be launched from **x86 Native Tools Command Prompt for VS 2022** (available after installing Build Tools). Pay attention not to use any other Command Prompt.
...
5. After installation, you can find **x86 Native Tools Command Prompt for VS 2022** in your Start Menu under "Visual Studio 2022" folder
...
Open **x86 Native Tools Command Prompt for VS 2022** (from Start Menu → Visual Studio 2022 → x86 Native Tools Command Prompt for VS 2022), go to ***BuildPath*** and run:
...
#### Build Debug version:
    ...
    msbuild ..\out\Telegram.sln /p:Configuration=Debug /p:Platform=Win32

#### Build Release version:

    msbuild ..\out\Telegram.sln /p:Configuration=Release /p:Platform=Win32

Alternatively, you can build just the Telegram project instead of the entire solution:

    msbuild ..\out\Telegram.vcxproj /p:Configuration=Release /p:Platform=Win32

### Build options

You can speed up the build by using parallel builds:

    msbuild ..\out\Telegram.sln /p:Configuration=Release /p:Platform=Win32 /m
...
Make sure you're running commands from **x86 Native Tools Command Prompt for VS 2022**, not from a regular Command Prompt or PowerShell window.
...
2. Configure with Ninja:
   ```
   configure.bat -D TDESKTOP_API_ID=YOUR_API_ID -D TDESKTOP_API_HASH=YOUR_API_HASH -G Ninja
   ```
